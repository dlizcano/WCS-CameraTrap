...
x4_bait <- cbind(as.data.frame(Tropero_Regional$bait), replicate(60, Tropero_Regional$bait))
x4_lat <- cbind(as.data.frame(Tropero_Regional$Latitud), replicate(60, Tropero_Regional$Latitud))
x4_country <- cbind(as.data.frame(Tropero_Regional$Country), replicate(60, Tropero_Regional$Country))
x4_CamType <- cbind(as.data.frame(Tropero_Regional$CamType), replicate(60, Tropero_Regional$CamType))
...
fit_stan_1T <- stan_occu(~1~1, data=umf, chains=3, iter=itera, cores=12)
fit_stan_2T <- stan_occu(~factor(bait)~1, data=umf, chains=3, iter=itera, cores=12)
fit_stan_3T <- stan_occu(~factor(CamTypes)~1, data=umf, chains=3, iter=itera, cores=12)
fit_stan_4T <- stan_occu(~scale(lat)~1, data=umf, chains=3, iter=itera, cores=12)
fit_stan_5T <- stan_occu(~factor(country)~1, data=umf, chains=3, iter=itera, cores=12)
fit_stan_6T <- stan_occu(~1~factor(country), data=umf, chains=3, iter=itera, cores=12)
fit_stan_7T <- stan_occu(~1~scale(lat), data=umf, chains=3, iter=itera, cores=12)
fit_stan_8T <- stan_occu(~1~scale(elevation), data=umf, chains=3, iter=itera, cores=12)
fit_stan_9T <- stan_occu(~1~factor(land_cover), data=umf, chains=3, iter=itera, cores=12)
fit_stan_10T <- stan_occu(~1~scale(per_tree_cov), data=umf, chains=3, iter=itera, cores=12)
fit_stan_11T <- stan_occu(~1~scale(road_den), data=umf, chains=3, iter=itera, cores=12)
fit_stan_12T <- stan_occu(~1~scale(cattle), data=umf, chains=3, iter=itera, cores=12)
...
# (fit_stan_2)
plot_effects(fit_stan_2T, "det") # Detection
plot_effects(fit_stan_3T, "det") # Detection
plot_effects(fit_stan_4T, "det") # Detection
plot_effects(fit_stan_5T, "det") # Detection
plot_effects(fit_stan_6T, "state") # Occupancy
plot_effects(fit_stan_7T, "state") # Occupancy
plot_effects(fit_stan_8T, "state") # Occupancy
plot_effects(fit_stan_9T, "state") # Occupancy
plot_effects(fit_stan_10T, "state") # Occupancy
plot_effects(fit_stan_11T, "state") # Occupancy
plot_effects(fit_stan_12T, "state") # Occupancy

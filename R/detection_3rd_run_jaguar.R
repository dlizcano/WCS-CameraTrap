...
#Jaguar_Regional <- read_csv("G:/detections_Jaguar_Bolivia_Venezuela_Peru_Guatemala_Paraguay_Ecuador.csv")
#y_reg <- Jaguar_Regional[,9:58]

#Jaguar_Bolivia <- detections_Jaguar_Bolivia <- read_csv("F:/BDcorregidas/Bolivia/Guido/Jaguar/detections_Jaguar_Bolivia.csv")
# y_reg <- Jaguar_Regional[,27:86]

# fix one coord
Jaguar_Regional3$longitude[2351] <- -73.182

# make sf
sites <- st_as_sf(Jaguar_Regional3, coords = c("longitude","latitude"))   #crs="EPSG:4326")
#--- set CRS ---#
st_crs(sites) <- 4326
...
# elevation_detailed <- rast(get_elev_raster(sites, z = 8, 
#                                             clip="bbox", 
#                                             neg_to_na=TRUE,
#                                             override_size_check = TRUE))
 
elevation_detailed <- get_elev_point (sites, 
                                      src="aws", 
                                      z = 5, # 7=1.2km, 8=600m
                                      overwrite=TRUE)
...
elev <- vector("numeric", length = nrow(sites))
for(i in seq_along(sites)){
  elev[i]<-get_elev_point(locations = sites[i,], 
                          prj = 4326, 
                          src = "aws", 
                          z = 8)$elevation
  }# end loop

# mts_elev <- cbind(sites, elev)
# mts_elev

###################
### Put elevation
# eleva <- terra::extract(elev, sites)
sites$elevation <- elevation_detailed$elevation #eleva$file3be898018c3 # elevation_detailed$elevation
####################

##### Filter by 2000 meters
ind4 <- which(sites$elevation >= 2000)
Jaguar_Regional4 <- Jaguar_Regional3[-ind4,]
y_reg4 <- y_reg3[-ind4,]
sites <- sites[-ind4,]
...
ind <- which(Jaguar_Regional$Jaguar_Design=="No")
# Jaguar_Regional$Jaguar_Design[ind] <- "no"
...
# camera_age
Jaguar_Regional4$camera_age <- Jaguar_Regional4$year_sampling - as.numeric(Jaguar_Regional4$year_purchased)
which(Jaguar_Regional4$camera_age<0)

# arrange detection observations

x4_bait <- cbind(as.data.frame(Jaguar_Regional4$bait), replicate(6, Jaguar_Regional4$bait))
x4_bait2 <- cbind(as.data.frame(Jaguar_Regional4$bait2), replicate(6, Jaguar_Regional4$bait2))
x4_lat <- cbind(as.data.frame(Jaguar_Regional4$latitude ), replicate(6, Jaguar_Regional4$latitude))
x4_country <- cbind(as.data.frame(Jaguar_Regional4$Pais), replicate(6, Jaguar_Regional4$Pais))
x4_CamType <- cbind(as.data.frame(Jaguar_Regional4$cam_brand), replicate(6, Jaguar_Regional4$cam_brand))
x4_Jaguar_Design <- cbind(as.data.frame(Jaguar_Regional4$Jaguar_Design), replicate(6, Jaguar_Regional4$Jaguar_Design))
x4n_cameras <- cbind(as.data.frame((Jaguar_Regional4$CamTypes)), replicate(6, (Jaguar_Regional4$CamTypes)))
x4n_season <- cbind(as.data.frame(as.factor(Jaguar_Regional4$season)), replicate(6, as.factor(Jaguar_Regional4$season)))
x4n_rio_playa <- cbind(as.data.frame(as.factor(Jaguar_Regional4$rio_playa)), replicate(6, as.factor(Jaguar_Regional4$rio_playa)))
x4n_arroyo <- cbind(as.data.frame(as.factor(Jaguar_Regional4$arroyo)), replicate(6, as.factor(Jaguar_Regional4$arroyo)))
x4n_camino <- cbind(as.data.frame(as.factor(Jaguar_Regional4$camino)), replicate(6, as.factor(Jaguar_Regional4$camino)))
x4n_senda_animal <- cbind(as.data.frame(as.factor(Jaguar_Regional4$senda_animal)), replicate(6, as.factor(Jaguar_Regional4$senda_animal)))
x4n_senda_gente  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$senda_gente)), replicate(6, as.factor(Jaguar_Regional4$senda_gente)))
x4n_salitral  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$salitral)), replicate(6, as.factor(Jaguar_Regional4$salitral )))
x4n_pozo_agua  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$pozo_agua)), replicate(6, as.factor(Jaguar_Regional4$pozo_agua )))
x4n_bosque  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$bosque)), replicate(6, as.factor(Jaguar_Regional4$bosque )))
x4n_sabana  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$sabana )), replicate(6, as.factor(Jaguar_Regional4$sabana )))
x4n_intermedio  <- cbind(as.data.frame(as.factor(Jaguar_Regional4$intermedio)), replicate(6, as.factor(Jaguar_Regional4$intermedio )))
x4n_camera_age  <- cbind(as.data.frame(as.numeric(Jaguar_Regional4$camera_age)), replicate(6, as.numeric(Jaguar_Regional4$camera_age )))
x4n_trigger_speed  <- cbind(as.data.frame(as.numeric(Jaguar_Regional4$trigger_speed)), replicate(6, as.numeric(Jaguar_Regional4$trigger_speed )))
...
fms1<-fitList("p(.) Ocu(.)"=            fit_1,
              ...
              "p(senda_animal_rio_playa) Ocu(.)"= fit_22,
              "p(camino_senda_animal_rio_playa) Ocu(.)"= fit_24,
              "p(camino_senda_animal_rio_playa_Jaguar_Design) Ocu(.)"= fit_25,
              "p(camino) Ocu(.)"=       fit_8,
...
pb3 <- parboot(fit_3, fitstats, nsim=300, report=1)
pb3
plot(pb3, main="p(CamTypes) Ocu(.)")

pb4 <- parboot(fit_4, fitstats, nsim=300, report=1)
pb4
plot(pb3, main="p(Jaguar_Design) Ocu(.)")


pb12 <- parboot(fit_12, fitstats, nsim=150, report=1)
pb12
plot(pb12, main="p(bait) Ocu(.)")



pb11 <- parboot(fit_11, fitstats, nsim=300, report=1)
pb11
plot(pb11, main="p(trigger_speed) Ocu(.)")
...
fit_stan_1 <- stan_occu(~1~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_2 <- stan_occu(~factor(bait)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_2b <- stan_occu(~factor(Jaguar_Design)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_3 <- stan_occu(~factor(CamTypes)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_3a <- stan_occu(~factor(n_cameras)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_4 <- stan_occu(~factor(rio_playa)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_5 <- stan_occu(~factor(arroyo)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_6 <- stan_occu(~factor(camino)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_7 <- stan_occu(~factor(senda_animal)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_8 <- stan_occu(~factor(senda_gente)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_9 <- stan_occu(~factor(salitral)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_10 <- stan_occu(~factor(pozo_agua)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_11 <- stan_occu(~factor(bosque)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_12 <- stan_occu(~factor(sabana)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_13 <- stan_occu(~factor(intermedio)~1, data=umf, chains=4, iter=itera, cores=8)
#fit_stan_14 <- stan_occu(~factor(country)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_15 <- stan_occu(~scale(camera_age)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_16 <- stan_occu(~factor(season)~1, data=umf, chains=4, iter=itera, cores=8)
fit_stan_17 <- stan_occu(~scale(trigger_speed)~1, data=umf, chains=4, iter=itera, cores=8)
...
stan_mods <- fitList("p(.) Ocu(.)" =           fit_stan_1,
                     "p(camino_senda_animal) Ocu(.)"=         fit_stan_20,
                     "p(Jaguar_Design) Ocu(.)"=fit_stan_2b,
                     "p(CamTypes) Ocu(.)"=     fit_stan_3,
                     "p(camino_rio_playa) Ocu(.)"=    fit_stan_21,
                     "p(rio_playa) Ocu(.)"=    fit_stan_4, #,
                     #"p(arroyo) Ocu(.)"=       fit_stan_5,#, 
                     "p(camino) Ocu(.)"=       fit_stan_6
                     # "p(senda_animal) Ocu(.)"= fit_stan_7)#,
...
# (fit_stan_2)
ubms::plot_effects(fit_stan_2, "det") + ggtitle("bait")# Detection
ubms::plot_effects(fit_stan_2b, "det") + ggtitle("Jaguar Design")# Detection
ubms::plot_effects(fit_stan_3, "det") + ggtitle("Camera Type")# Detection
ubms::plot_effects(fit_stan_4, "det") + ggtitle("Rio Playa")# Detection
ubms::plot_effects(fit_stan_5, "det") + ggtitle("Arroyo")# Detection
ubms::plot_effects(fit_stan_6, "det") + ggtitle("Camino")# Detection
ubms::plot_effects(fit_stan_7, "det") + ggtitle("Senda Animal")# Detection
ubms::plot_effects(fit_stan_8, "det") + ggtitle("Senda Gente")# Detection
ubms::plot_effects(fit_stan_9, "det") + ggtitle("Salitral")# Detection
ubms::plot_effects(fit_stan_10, "det") + ggtitle("Pozo Agua")# Detection
ubms::plot_effects(fit_stan_11, "det") + ggtitle("Bosque")# Detection
ubms::plot_effects(fit_stan_12, "det") + ggtitle("Sabana")# Detection
ubms::plot_effects(fit_stan_13, "det") + ggtitle("Intermedio")# Detection
ubms::plot_effects(fit_stan_14, "det") + ggtitle("Pais")# Detection
ubms::plot_effects(fit_stan_15, "det") + ggtitle("Camera Age") # Detection
ubms::plot_effects(fit_stan_16, "det") + ggtitle("Season") # Detection
ubms::plot_effects(fit_stan_17, "det") + ggtitle("Trigger Speed") # Detection



ubms::plot_effects(fit_stan_9, "state") + ggtitle("Land cover") # Occu
ubms::plot_effects(fit_stan_10, "state") + ggtitle("Percent tree cover") # Occu
ubms::plot_effects(fit_stan_11, "state") + ggtitle("Road density") # Occu
ubms::plot_effects(fit_stan_12, "state") + ggtitle("Cattle density") # Occu
ubms::plot_effects(fit_stan_13, "state") + ggtitle("River") # Occu
